...
- [ ] Design zero-copy byte-slicing API surface for `tpt-lexicon-core` (buffer ownership model, lifetimes, caller-supplied allocator story for `no_std`)
- [ ] Implement baseline (sequential) BPE tokenizer as a correctness reference
...
- [ ] Vocabulary loading format (internal binary layout, versioned)
- [ ] Unit tests: round-trip encode/decode correctness against reference BPE
...
- [ ] `tpt-lexicon-ingest`: streaming parser core (chunk iterator API, no full-string buffering)
- [ ] `tpt-lexicon-ingest`: Tree-sitter interop for Rust
- [ ] `tpt-lexicon-ingest`: Tree-sitter interop for TypeScript
- [ ] `tpt-lexicon-ingest`: Markdown boundary detection
- [ ] `tpt-lexicon-ingest`: JSON boundary detection
- [ ] `tpt-lexicon-ingest`: natural-language paragraph boundary detection
- [ ] `tpt-lexicon-ir`: typed AST/IR schema definition
- [ ] `tpt-lexicon-ir`: serialization format (serde or custom, versioned, diffable)
- [ ] `tpt-lexicon-ir`: boilerplate/repetition detection for structured data (code, JSON, logs)
- [ ] `tpt-lexicon-ir`: recursive grammar-rule compression for detected sub-graphs
- [ ] `tpt-lexicon-ir`: standard semantic pooling path for open-ended natural language (per Risk mitigation, spec §8)
- [ ] Tests: compression round-trips losslessly (decompressed IR == original structure)
...
- [ ] `tpt-lexicon-translate`: JIT unrolling of compressed IR into discrete legacy token-ID sequences
- [ ] `tpt-lexicon-translate`: HuggingFace `tokenizer.json` loader
- [ ] `tpt-lexicon-translate`: map loaded HF vocabularies into `tpt-lexicon-core` zero-copy structures
- [ ] `tpt-lexicon-translate`: cross-IR export — Tree-sitter AST
- [ ] `tpt-lexicon-translate`: cross-IR export — LSP
- [ ] `tpt-lexicon-translate`: cross-IR export — LLVM IR
...
- [ ] `tpt-lexicon-verify`: define structural invariant set (balanced delimiters, valid type references, borrow-rule validity, etc.)
- [ ] `tpt-lexicon-verify`: formal proof contracts for IR edits (spec the proof system/approach — e.g. SMT-backed, type-driven, or custom checker)
- [ ] `tpt-lexicon-verify`: rejection path wired into `tpt-lexicon-translate` before text rendering
- [ ] Tests: adversarial/malformed IR inputs are all rejected; well-formed IR always passes
...
- [ ] Confirm core stack builds and runs correctly with `tpt-lexicon-gpu` entirely excluded (**G6** composability check)
...
- [ ] **G6** Composable crate surface — every crate builds/tests standalone; `gpu` fully optional (ongoing, final check in Phase 4)
...
- [ ] Crate-level `#![deny(missing_docs)]` (or equivalent) satisfied — no undocumented public API surface
